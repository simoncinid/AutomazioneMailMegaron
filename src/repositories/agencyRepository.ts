...
export function buildMatchKey(row: Pick<AgencyInsertRow, 'external_agency_id'>): string {
  return row.external_agency_id && row.external_agency_id.trim().length > 0
    ? row.external_agency_id
    : '__no_external_id__';
}

export function computeContentHash(row: AgencyInsertRow): string {
...
async function loadExisting(
  ...
  const result = await pool.query<{
    ...
    `SELECT id,
            COALESCE(external_agency_id, '__no_external_id__') AS match_key,
            content_hash
     FROM gestim_agencies
...
async function bulkInsert(items: Array<{ row: AgencyInsertRow; hash: string }>): Promise<number> {
...
async function bulkUpdate(items: Array<{ id: number; row: AgencyInsertRow; hash: string }>): Promise<number> {
  if (items.length === 0) return 0;
  const pool = getPool();
  ...
  const sql = `
    UPDATE gestim_agencies AS a SET
    ...
    WHERE a.id = u.id
  `;
  const result = await pool.query(sql, [ids, importRunIds, names, statuses, emails, phones, rawJsons, hashes]);
  return result.rowCount ?? 0;
}

export async function upsertMany(
  rows: AgencyInsertRow[],
  agencyCode: string | null,
  siteCode: string | null
  ...
  const toInsert: Array<{ row: AgencyInsertRow; hash: string }> = [];
  const toUpdate: Array<{ id: number; row: AgencyInsertRow; hash: string }> = [];
